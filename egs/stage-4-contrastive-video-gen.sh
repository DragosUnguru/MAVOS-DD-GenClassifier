#!/bin/bash
# Stage 4 – Video Generative Method Contrastive Classification
#
# Trains VideoCAVMAEContrastive with a 5-class head:
#   0=real, 1=memo, 2=liveportrait, 3=inswapper, 4=echomimic
set -e
...
save_dir=/mnt/d/projects/MAVOS-DD-GenClassifer/checkpoints/contrastive_two_steps_adversarial_gen_method_classification_head_MINISET
mkdir -p $save_dir
...
CUDA_CACHE_DISABLE=1 python -W ignore ../src/run_ft_contrastive_video_gen.py \
    --data-train ${tr_data} --data-val ${te_data} --save-dir $save_dir --n_classes 5 \
    ...
    --supcon_weight $SUPCON_WEIGHT --cls_weight $CLS_WEIGHT \
