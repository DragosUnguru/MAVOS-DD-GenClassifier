...
parser.add_argument("--masking_rate", type=float, default=0.)
parser.add_argument("--masking_strategy", type=str, default="uniform")
...
wandb.init(project="AVFF deepfake detection", 
               config=vars(args), 
               name=f"Masking ratio: {args.masking_rate} {args.masking_strategy}")

input_path = args.input_path
mavos_dd = datasets.Dataset.load_from_disk(input_path)

train_loader = DataLoader(
    MavosDD(load_from_disk("subset"), input_path, audio_conf, stage=2),
...
# Construct model
cavmae_ft = VideoCAVMAEFT(masking_strategy=args.masking_strategy)
