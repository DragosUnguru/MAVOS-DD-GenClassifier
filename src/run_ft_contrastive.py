...
parser = argparse.ArgumentParser(description='Video CAV-MAE Contrastive Training')
...
parser.add_argument('--save-dir', default='checkpoints/contrastive_training', type=str, help='directory to save checkpoints')
...
# Contrastive training specific arguments
parser.add_argument('--temperature', type=float, default=0.07, help='temperature for contrastive loss softmax scaling')
parser.add_argument('--lambda_domain', type=float, default=1.0, help='weight for domain confusion loss')
...
print(f'  - Lambda domain confusion: {args.lambda_domain}')
...
# Load model with contrastive training support
...
# Train model with contrastive training
print("Now start contrastive training for %d epochs" % args.n_epochs)
train_contrastive(cavmae_ft, train_loader, val_loader, args)
