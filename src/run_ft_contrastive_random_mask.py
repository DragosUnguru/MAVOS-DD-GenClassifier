...
from models.video_cav_mae import VideoCAVMAEContrastiveRandomMask
...
parser = argparse.ArgumentParser(description='Video CAV-MAE Contrastive Learning with Random Masking')
...
# Model arguments
parser.add_argument('--save-dir', default='checkpoints/contrastive_random_mask', type=str)
...
# Random masking
parser.add_argument('--apply_mask', type=bool, default=True, help='whether to apply random masking')
parser.add_argument('--mask_ratio', type=float, default=0.4, help='ratio of visual tokens to randomly mask')
...
print('='*60)
print('Contrastive Learning with Random Masking Configuration:')
print('='*60)
...
print(f'  - Apply random masking: {args.apply_mask}')
print(f'  - Mask ratio: {args.mask_ratio}')
print(f'  - No MaskingNet (random masking)')
print(f'  - No adversarial game (single-step training)')
print('='*60)

# Class mappings (used for dataset loading compatibility)
video_labels = {
    "memo": 0,
    "liveportrait": 1,
    "inswapper": 2,
...
model = VideoCAVMAEContrastiveRandomMask(
...
print("Starting contrastive learning with random masking for %d epochs" % args.n_epochs)
train_contrastive_random_mask(model, train_loader, val_loader, args)
