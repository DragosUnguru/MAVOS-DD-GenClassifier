...
from traintest_ft import train_adversarial
import warnings

from mavosdd_dataset_multiclass import MavosDD, get_audio_label


parser = argparse.ArgumentParser(description='Video CAV-MAE Adversarial Training')
...
parser.add_argument('--n_classes', default=2, type=int, help='Num of classes to be classified (real/fake)')
parser.add_argument('--save-dir', default='checkpoints/adversarial_training', type=str, help='directory to save checkpoints')
...
parser.add_argument('--mask_loss_lambda', type=float, default=0.2, help='weight for mask regularization loss')
...
parser.add_argument('--lambda_adv', type=float, default=1.0, help='gradient reversal strength for adversarial training')
parser.add_argument('--gen_loss_weight', type=float, default=1.0, help='weight for generative method classification loss')
...
parser.add_argument("--miniset", type=bool, default=False, help="use mini dataset for quick evaluation")
...
print(f'  - Mask loss lambda: {args.mask_loss_lambda}')
...
if args.miniset:
    ...
    train_loader = DataLoader(
        MavosDD(
            dataset=mavos_dd.filter(lambda sample: sample['split'] == "train"), # and (sample['generative_method'] != "real" or sample['audio_generative_method'] != "real")
            input_path=input_path,
            audio_conf=audio_conf,
            stage=2,
            video_class_name_to_idx=video_labels,
...
print(f"Using Train: {len(train_loader)} batches, Eval: {len(val_loader)} batches")
print(f"Number of classes for generative method classification: {len(all_labels)}")

# Load model with adversarial training support
...
print("Now start adversarial training for %d epochs" % args.n_epochs)
train_adversarial(cavmae_ft, train_loader, val_loader, args)
